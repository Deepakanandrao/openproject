 ...
 * `"PROJ-7"`) to the internal primary key we use for cache, selection,
 * focus, and API calls.
 *
 ...
 * a numeric PK, silently corrupting comparisons and lookups. Callers
 * must handle the `null` case explicitly.
 *
 * The cache is currently keyed only by numeric PK, so semantic route
 * params will reliably miss until the cache is dual-keyed. Treat this
 * helper as a temporary shim around that limitation.
 */
export function resolveWorkPackagePk(states:States, routeParam:string):string | null {
