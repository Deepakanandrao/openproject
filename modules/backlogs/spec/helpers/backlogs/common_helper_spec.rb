...
RSpec.describe Backlogs::CommonHelper do
  ...
  describe "#filter_fields_for" do
    let(:params) { { bucket_ids: %w[1 2], sprint_ids: %w[3], all: "1" } }

    it "excludes bucket_ids when selector_type is :buckets" do
      names = helper.filter_fields_for(:buckets).map { |i| i[:name].to_s }
      expect(names).not_to include("bucket_ids[]")
      expect(names).to include("sprint_ids[]", "all")
    end

    it "excludes sprint_ids when selector_type is :sprints" do
      names = helper.filter_fields_for(:sprints).map { |i| i[:name].to_s }
      expect(names).not_to include("sprint_ids[]")
      expect(names).to include("bucket_ids[]", "all")
    end
  end
end
